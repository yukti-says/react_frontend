...
import { subscribeToTodos, updateTodo } from "./todos"; // add updateTodo in todos.js
...
const App = () => {
  ...
  const [todos, setTodos] = useState([]); // User's tasks
  const [editingTodo, setEditingTodo] = useState(null); // Todo currently being edited
  ...
  // Subscribe to todos only if user is logged in
  useEffect(() => {
    if (!user) return;
  ...
  // Save edits
  const handleSaveEdit = async (todoId, newText) => {
    if (!todoId || !newText) {
      setEditingTodo(null); // cancel
      return;
    }
    await updateTodo(todoId, { text: newText });
    setEditingTodo(null); // reset after save
  };

  return (
    <div className="min-h-screen bg-gradient-to-r from-lime-500 via-emerald-600 to-green-900 flex flex-col items-center p-6">
        ...
        <div className="w-full max-w-2xl">
          <div className="flex justify-between items-center mb-4 text-white">
            <p className="text-lg font-semibold">
              Welcome, {user.email.split("@")[0].split(".")[0]}
            </p>
            <button
              onClick={logoutUser}
          ...
          <TodoList todos={todos} onEdit={(todo) => setEditingTodo(todo)} />
        </div>
      )}
    </div>
  );
